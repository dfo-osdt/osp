...
<template>
    <BaseDialog :title="$t('create-publication-dialog.title')" persistent>
        <QStepper ref="stepper" v-model="step" color="primary" animated flat bordered>
            <q-separator />
            <q-step :name="1" :title="$t('create-publication-dialog.before-your-start')" icon="mdi-call-split"
            ...
            <q-step :name="2" :title="$t('create-publication-dialog.publication-details')"
                ...
                <QForm ref="detailsForm" class="q-ma-md">
                    ...
                    <q-input v-model="title" :label="$t('common.title')" class="q-mb-md" outlined :rules="[
                        (val) => val.length > 0 || $t('common.required'),
                    ]" />
                    <JournalSelect v-model="journalId" class="q-mb-md" :rules="[
                        (val: number) =>
                            val !== null || $t('common.required'),
                    ]" />
                    <DoiInput v-model="doi" class="q-mb-md" />
                    <RegionSelect v-model="regionId" :label="$t('common.lead-region')" outlined
                        :rules="[(val: number) => !!val || $t('common.required')]"
                        :hint="$t('publication.lead-region-hint')" class="q-mb-md" />
                </QForm>
            </q-step>
            <q-step :name="3" :title="$t('create-publication-dialog.dates-and-access')" icon="mdi-calendar"
                ...
                <QForm ref="datesForm">
                    ...
                    <q-separator class="q-mb-md" />
                    <DateInput v-model="acceptedOn" :label="$t('create-publication-dialog.accepted-on-optional')
                        " class="q-mb-md" />
                    <DateInput v-model="publishedOn" :label="$t('create-publication-dialog.published-on')"
                    ...
                    <DateInput v-if="!isOpenAccess" v-model="embargoedUntil" :label="$t('common.embargoed-until')"
                        :required="!isOpenAccess" />
                </QForm>
            </q-step>
            <template #navigation>
                <q-stepper-navigation class="flex justify-end">
                    <q-btn color="primary" :label="step === 3 ? $t('common.create') : $t('common.next')
                        " class="q-mr-sm" @click="next()" />
                    <q-btn v-if="step > 1" flat color="primary" :label="$t('common.back')"
